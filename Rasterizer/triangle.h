...
class triangle {
    ...
    vec2D e0, e1, e2;  // edge vectors for interpolation
...
public:
    ...
    triangle(const Vertex& v1, const Vertex& v2, const Vertex& v3) {
        ...
        // Calculate and STORE edge vectors
        e0 = vec2D(v[1].p - v[0].p);  // alpha
        e1 = vec2D(v[2].p - v[1].p);  // beta
        e2 = vec2D(v[0].p - v[2].p);  // gamma
    ...
    void draw(Renderer& renderer, Light& L, float ka, float kd) {
        ...
        //Calculate step value across in y
        float alpha_step = -e0.y / area;
        float beta_step = -e1.y / area;
        float gamma_step = -e2.y / area;

        //Ambient colour
        colour colAmbient = L.ambient * ka;

        // Iterate over the bounding box and check each pixel
        for (int y = (int)(minV.y); y < (int)ceil(maxV.y); y++) {
            //Calculate first a/b/g per row
            vec2D pixel1 ((float)minV.x, (float)y);
            float alpha = 
            float beta = 
            ...
            for (int x = (int)(minV.x); x < (int)ceil(maxV.x); x++) {
                ...
                if (getCoordinates(vec2D((float)x, (float)y), alpha, beta, gamma)) {
                    ...
                    if (renderer.zbuffer(x, y) > depth && depth > 0.001f) {
                        ...
                        //COMMENTED OUT NORMALISE
                        //L.omega_i.normalise(); 
                        float dot = std::max(vec4::dot(L.omega_i, normal), 0.0f);
                        colour a = (c * kd) * (L.L * dot) + colAmbient; // using kd instead of ka for ambient
                        // typical shader end
                        unsigned char r, g, b;
                        a.toRGB(r, g, b);
